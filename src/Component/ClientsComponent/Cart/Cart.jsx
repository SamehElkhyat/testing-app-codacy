...
import { Table, Form, Button, Row, Modal, Col, Card } from "react-bootstrap";
...
const Portfolio = () => {
  ...
  function WalletInfo() {
    ...
    const InformationAboutOrder = async (page = 1) => {
      try {
        const { data } = await axios.get(
          `${process.env.REACT_APP_API_URL_MICROSERVICE2}/Get-Count-Accept-Failed-Wait-Orders-Broker/${page}`,
          {
    ...
    useEffect(() => {
      InformationAboutOrder(currentPage );
    }, []);
    ...
    return (
      <motion.div
        ...
        exit={{ opacity: 0 }}
      >
        <Row className="flex flex-col justify-center w-full m-2 gap-4">
          <p className="text-end text-[var(--secondColor--)] font-bold text-lg sm:text-xl p-2">
            حاله الطلبات
          </p>
          <div className="first-col-use flex flex-col md:flex-row justify-start w-full gap-3">
          ...
          <div className="second-col-usr flex flex-col md:flex-row justify-end w-full gap-3">
            <div className="box p-2 w-full md:w-1/3">
              <Col className="mb-3">
                <Link className="text-white text-decoration-none">
                  <Card className="p-3">
          ...
          <Table striped bordered hover className="text-sm sm:text-base">
            <thead>
              <tr>
                ...
                <th className="text-xs sm:text-sm md:text-base">الملاحظات</th>
              ...
              {loading ? (
                ...
                filteredData.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td className="text-xs sm:text-sm md:text-base">{order.notes}</td>
