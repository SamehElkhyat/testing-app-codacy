...
const PendingOrders = () => {
  ...
  const GetOrder = async (page = 1) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL_MICROSERVICE2}/Get-Orders/${page}`,
        {
      ...
      if (res.data.data && res.data.totalPages !== undefined) {
        // Paginated response
  ...
  const handleChangeId = async (value) => {
    console.log(value);
    ...
    try {
      ...
      navigate(`/OrderDetailsForUser/${data}`);
    } catch (error) {}
  };
