  ...
  const GetBlackList = async (page = 1) => {
    ...
    try {
      ...
      if (response.data.data && response.data.totalPages !== undefined) {
        ...
        setTotalBlocked(response.data.totalBlocked || response.data.data.length);
      } else {
  ...
  return (
    <>
      <Box width="100%" textAlign="center" p={4}>
        ...
        {/* Blocked users count info */}
        <div className="mb-4 text-center">
          <p className="text-white text-sm sm:text-base">
            إجمالي المحظورين:{" "}
            <span className="font-bold text-blue-300">{totalBlocked}</span>
          </p>
        </div>
          ...
          <div className="w-100 max-w-2xl p-4">
            <div className="flex items-center justify-between border border-blue-300 rounded-lg px-4 py-2 bg-white shadow-sm focus-within:ring-2 focus-within:ring-blue-400">
              <input
                ...
                type="text"
                placeholder="ابحث عن محظور (الاسم، البريد، الهوية، الهاتف)"
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="text-center">
                    {searchTerm ? "لا توجد نتائج للبحث" : "لا توجد محظورين للعرض"}
                  </td>
                </tr>
              ) : (
