  ...
  const CustomerService = async (page = 1) => {
    ...
    try {
      ...
      if (data.data && data.totalPages !== undefined) {
        ...
        setTotalCustomerService(data.totalCustomerService || data.data.length);
      } else {
        // Non-paginated response (fallback)
        setSelectedOrder(data);
        setTotalPages(1);
        setTotalCustomerService(data.length);
      }
    } catch (error) {
      console.log(error);
    } finally {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Customer Service count info */}
        <div className="mb-4 text-center">
          <p className="text-white text-sm sm:text-base">
            إجمالي موظفي خدمة العملاء:{" "}
            <span className="font-bold text-blue-300">{totalCustomerService}</span>
          </p>
        </div>
          ...
          <div className="w-100 max-w-2xl p-4">
            <div className="flex items-center justify-between border border-blue-300 rounded-lg px-4 py-2 bg-white shadow-sm focus-within:ring-2 focus-within:ring-blue-400">
              <input
                ...
                type="text"
                placeholder="ابحث عن موظف خدمة عملاء (الاسم، البريد، الهوية، الهاتف)"
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="text-center">
                    {searchTerm ? "لا توجد نتائج للبحث" : "لا توجد موظفي خدمة عملاء للعرض"}
                  </td>
                </tr>
              ) : (
